...
class TrainPipeline:
    ...
    def start_model_trainer(self, data_validation_artifact: DataValidationArtifact) -> ModelTrainerArtifact:
        try:
            logging.info(
                "Entered start_model_trainer method of TrainPipeline class")
            ...
            logging.info(
                "Exited start_model_trainer method of TrainPipeline class")
    ...
    def run_pipeline(self):
        try:
            ...
            # Step 3: Model Training (only if data is valid)
            if data_validation_artifact.validation_status == True:
                model_trainer_artifact = self.start_model_trainer(
                    data_validation_artifact=data_validation_artifact
                )
                logging.info(
                    f"Training complete. Model saved at: {model_trainer_artifact.trained_model_file_path}")
            else:
