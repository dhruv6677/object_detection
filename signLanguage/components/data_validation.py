...
class DataValidation:
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            validated_data_dir = ""
            if status:
                feature_src = self.data_ingestion_artifact.feature_store_path
                feature_dst = os.path.join(
                    self.data_validation_config.data_validation_dir,
                    os.path.basename(feature_src),
                )
                shutil.copytree(feature_src, feature_dst, dirs_exist_ok=True)
                validated_data_dir = feature_dst
            data_validation_artifact = DataValidationArtifact(
                validation_status=status,
                validated_data_dir=validated_data_dir,
            )
            logging.info(f"Exited initiate_data_validation method of DataValidation class")
